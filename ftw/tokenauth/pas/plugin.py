...
def addTokenAuthenticationPlugin(
    self,
    id_,
    title=None,
...
class TokenAuthenticationPlugin(BasePlugin):
    ...
    manage_config = PageTemplateFile(
        "www/config", globals(), __name__="manage_config"
    )
    ...
    def authenticateCredentials(self, credentials):
        ...
        if ip_range is not None:
            ...
            if not client_ip:
                ...
                log.warn(
                    "Authentication attempt for key with IP range "
            ...
            if not permitted_ip(client_ip, ip_range):
                log.warn(
                    "Authentication attempt from disallowed IP %s" % client_ip
                )
                return None
    ...
    def manage_updateConfig(self, REQUEST):
        ...
        self.access_token_lifetime = int(
            REQUEST.form.get("access_token_lifetime", DEFAULT_TOKEN_LIFETIME)
        )

        self.usage_log_retention_days = int(
            REQUEST.form.get(
